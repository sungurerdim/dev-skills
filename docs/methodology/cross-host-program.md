...
## 7. ds-rig (29th skill, shipped 2026-07-15)

New Equip-family skill built from the toolset research (`/tmp/ds-skills-research/toolset-skill.md`): machine-level rig — pinned tool install/update with drift table on re-run, zero-telemetry hardening proven by config read-back, additive allow/ask/deny permission profiles per harness, and the MCP tool-count token budget (~20-30 threshold) as its novel contribution (no surveyed prior-art tool does this). Design decisions recorded: name `ds-rig` (rig > toolset — scope includes privacy + permissions, not just tools) · manifest at `~/.config/ds-rig/manifest.json` (machine-global record; state-exempt, no `ds/audit/`) · context-mode catalogued with mandatory per-item confirmation of its maintainer-acknowledged sandbox-escape caveat · never-silent classes (unpinned `npx -y`/`curl|bash`, credential-passthrough MCPs, self-updaters, harness-config writes, budget-crossing adds) require per-item confirmation. Taxonomy: new dimension D11 added via the amendment process (SKILL-SPEC appendix + CLAUDE.md family map + this doc). Boundary vs ds-init kept deliberately: machine layer (rig) vs project layer (init) — merging would violate Trigger Discipline and the focused-skill evidence (F3); cross-references added to both trigger tables.

Raw research artifacts (session-local, not committed): `/tmp/ds-skills-research/{competitors,harnesses,models,toolset-skill,family-*}.md`. Durable summary: knowledge repo, `repos/dev-skills/`. Raw research data for the "harness system prompts" pass (root instruction-file content guidance across CLAUDE.md/AGENTS.md/.cursor/rules/.windsurfrules/CONVENTIONS.md/copilot-instructions.md/GEMINI.md): `docs/methodology/ai-harness-context-files-research.json`.

## 8. Lean install profile (v6.1, shipped 2026-08-11)

The Claude-5-only distribution that F1/F4/F6 and SKILL-SPEC §1 sanctioned but nothing built now exists as `./install.sh --profile lean`: an install-time strip of `<!-- portable-only -->`-marked blocks (closing Completion Evidence band, generic W-recap line, pre-existing-errors contract bullet — each restating a gate the lean host's always-on rules layer already supplies). Repo files always keep full portable text; `--check` is profile-aware via the version stamp; consistency checks 27 (marker hygiene) + 28 (principles.md majority-hash sync with declared variants) and five new install round-trip assertions gate it. Same session: in-file dedup (Error Ownership 3→1 per skill, FRC/DSC acronyms → self-describing canonical strings, dead SKILL-SPEC citations → inline meaning), SKILL-SPEC §2 recalibrated to the Claude-5 generation with the Model-Class Calibration table (frontier → lean density; DeepSeek-V4-class/thin-harness → portable density), and run-time currency rules added to dated-fact references (ds-launch, ds-mobile, ds-compliance). Evidence base: 2026-08-11 research pass — Databricks harness benchmark, DeepSeek V4-Flash-0731 measurements, Anthropic Fable-5 prompting doc; durable copy in the knowledge repo under `repos/dev-skills/`.
