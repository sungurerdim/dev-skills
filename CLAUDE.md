...
- **Mutation suite proves *which* check fired:** every `--mutation-test` case declares the ERE its own check's `err()` emits; a red gate whose FAIL lines match nothing reports `MUTATION MISPOINTED` instead of passing, so a mutated check that silently no-ops can no longer hide behind an unrelated failure.
