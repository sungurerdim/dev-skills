...
- **Impact:** 2.5B people worldwide have disabilities. Keyboard navigation is the foundation of all assistive technology access. Missing keyboard access = legal liability under EAA (June 2025), ADA, AODA.
...
- **Impact:** Without focus indicator, keyboard users cannot see where they are on the page. WCAG 2.2 SC 2.4.11 requires minimum focus appearance.
...
- **Fix:** Adjust either text or background color to meet ratio. Use WCAG contrast formula. Prefer adjusting the lighter color (more perceptually stable). Tools: WebAIM Contrast Checker, Chrome DevTools contrast inspector.
- **Impact:** 83.6% of websites fail color contrast (WebAIM 2024). It is the number one accessibility violation. Error rate increases significantly with low contrast, especially for low-vision users (14% of adults).
...
- **Fix:** Add accessible name. Prefer visible text over aria-label. For icon-only buttons: `aria-label="Delete item"` (web), `tooltip: 'Delete item'` (Flutter), `.accessibilityLabel("Delete item")` (SwiftUI), `contentDescription = "Delete item"` (Compose). For decorative images: `alt=""` (web), excludeFromSemantics (Flutter).
...
- **Fix:** Increase padding/size to meet minimum. For icon buttons: add padding around icon to reach target. Mobile: 48x48dp minimum (Material), 44x44pt (Apple HIG).
...
- **Impact:** Sighted users see error messages appear. Screen reader users hear nothing unless errors are announced via live regions. Silent failures = abandoned forms.
...
- **Impact:** Motion can trigger vestibular disorders (dizziness, nausea) in affected users. Respecting the preference is both an accessibility and comfort requirement.
