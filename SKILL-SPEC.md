...
core/             <- Shared references, linked from any skill as ../core/<file>.md (shipped on every install)
```

**Standalone with core.** A skill is complete when installed alone *together with* `core/` — the installer ships `core/` on every install (full, `--skills` subset, `--target`), so `../core/<file>.md` resolves in a lone install exactly as in the repo. A skill links to `core/` files by relative path and never copies their text; it never links to another skill's directory, and it never cites this spec — every rule a skill relies on at runtime lives in the skill or in `core/` (see §9 Prohibited Content). Cross-skill *prose* handoffs remain the advisory-handoff pattern (§12).
...
| 4 | Arguments | Yes | Flags, modes, defaults. MUST include `--ask` with the canonical row (see §2 Autonomous Default) — no skill-local variant. |
...
- `[CONDITION]` — phase-level entry condition (e.g., `[--ask]`, `[findings > 0]`)
...
See the [AI Instruction Patterns appendix](#appendix-ai-instruction-patterns--research-rationale) for the research.
...
### Autonomous Default and `--ask`

Every skill runs autonomously by default. `--ask` is the single, universal flag that restores the interactive flow; every `ds-*` skill's Arguments table carries it with the canonical row below and no skill re-derives, narrows, or renames it. No skill exposes a "suppress prompts" / "apply everything" flag — the default already does that — and no skill exposes a skill-local "ask about this one thing" flag: `--ask` is the only flag on this axis.
...
| Flag | Meaning | Notes |
|------|---------|-------|
| `--ask` | Interactive run | See Autonomous Default above. The only flag on this axis; `--auto` is retired. |
...
Every menu, list, or selection point that a skill presents to the user under `--ask` MUST include an **"all"** affordance (synonym: `apply-all`, `approve-all`, `all matching`) when more than one item is available.
...
Every **multi-mode / multi-function** skill (more than one mode, scope, or operation) MUST present the choice **up front under `--ask`** when the user does not pass a disambiguating flag. Without `--ask` the menu is never shown: the skill selects its best-judgment default and records the choice in the summary.
...
Every point where the skill asks the user to choose or approve (under `--ask`) MUST make **exactly what is being asked** unmistakable, and present the items **compactly and completely** so approval is transparent.
...
**Applies to:** ds-ship, ds-build, ds-issue, ds-freeze (any multi-phase run).
...
**Applies to:** ds-ship, ds-build, ds-issue, ds-research, ds-brief, ds-freeze (skills that delegate to phases or subagents).
...
The staging-exclusion filename set and the content-regex family have exactly one home: [`core/secret-patterns.md`](core/secret-patterns.md). Any skill that carries a filename-based exclusion carries the full canonical set verbatim (`check-consistency.sh` check `secret-pattern-set`); a partial copy is drift — the file a partial list misses is the one that leaks.

### Checkpoint Gate (bulk-modifying skills)

Every skill whose flow writes, deletes, or resets project files in bulk runs the clean-tree pre-gate in [`core/checkpoint-protocol.md`](core/checkpoint-protocol.md) before its first write; a flow whose failure path reverts or resets tracked files treats it as stop-hard. Rationale: running a revert-capable flow over uncommitted user work is a data-loss path. `check-consistency.sh` (check `checkpoint-gate`) enforces presence for the named bulk-modifying skills.
...
### Severity, Confidence, Score, Skip Patterns, Categories

One home: [`core/severity-score-categories.md`](core/severity-score-categories.md) — four severity levels (CRITICAL / HIGH / MEDIUM / LOW, plus BLOCKER / ADVISORY / INFO in rule files; MAJOR/MINOR banned), the second-pass rule for CRITICAL, confidence bands, the score formula with its caps, skip patterns, false-positive prevention, fix categories A/B and finding categories CAT-1/CAT-2, and the disposition vocabulary. No skill re-derives a local scale (`check-consistency.sh` check 15 enforces the vocabulary in rule files).
...
In practice only long autonomous skills qualify. The qualifying set is enumerated in exactly one place — the `allowed` list of `scripts/check-consistency.sh` check 5 — and that check enforces it in both directions (a non-qualifying skill that grows the protocol fails; a qualifying skill that loses it fails). Members earn their place on the test above: `ds-tune` (experiment loops), `ds-ship` (multi-phase orchestration), `ds-blueprint` (full-codebase scoring), `ds-mobile` (13-domain audit) and `ds-frontend` (multi-scope audit) — a scan that walks scope after scope holds its progress in the run alone, so an interruption costs the whole scan. Everything else is exempt.
...
**Recovery-check exemption from `[SKIP]` annotations:** Even when a phase is annotated `[--ask]` or `[SKIP if {flag}]`, the Recovery Check (step 1 of Setup phase for qualifying skills) MUST execute unconditionally. Annotate the phase header `[--ask — except step 1 Recovery Check]` so the AI does not skip the recovery step on a default run. A skipped recovery on a resumed session causes silent state corruption.
...
2. **READ STATE** — Parse the JSON.
   - If `git_hash` ≠ current HEAD → default: continue (resuming is the best-judgment default — re-verification in step 3 catches genuine drift) and note the mismatch; `--ask` → warn: `State from commit {state_hash} differs from HEAD {current}. Source-reading phases will re-verify. Resume? [Y/n]`. User N → delete state, fresh start.
...
| `--resume` not given, state exists | Default: resume (no prompt), announce it. `--ask`: `Resume previous run? [Y/n]`. N → delete + fresh. Y → resume. |
...
**Exempt skills — everything outside the qualifying set.** Every other skill is exempt and states the one-line exemption reason in its Contract section. Canonical exemption classes:
...
Skills may add domain-specific modes (e.g., `release-ready`, `design`). Mode selection is presented as a menu under `--ask` when no flag is provided; otherwise the skill selects its best-judgment default without showing the menu and records the choice.
...
Each skill is **fully functional standalone**. Skills are self-contained: no skill references this spec, another skill's files, or anything outside its own directory and `core/` at runtime (`core/` ships with every install — §1 Standalone with core). The only shared *runtime data* is `ds/audit/findings.md` (repo-root `ds/audit/` directory) and the blueprint profile section in the AI instruction file — both are optional optimizations.
...
Skills do not import from each other. Shared *text* lives in `core/` (one copy, linked by relative path); shared *data* flows through two well-known file locations:
...
The schema, meta header, freshness rule, write semantics, consumption rule, producer priority and dedup rule have exactly one home: [`core/findings-and-profile-format.md`](core/findings-and-profile-format.md) § 1. Skills link there; this spec does not restate it. The `Category` column (A/B) follows the fix categories in `core/severity-score-categories.md`.
...
| Citations of this spec (`SKILL-SPEC`, `Autonomous Default rule N`, `rule-4 exception`, `IDU`, `DSC`, `OVERLAP-n`, `§N`) | A lone install has no spec to resolve them against; the executor is left with a pointer and no rule. Enforced by `check-consistency.sh` check `spec-citations` |

**Canonical inline equivalents** — a skill says the rule, never the citation:

| Instead of | Write |
|-----------|-------|
| `resolve per best judgment` | `resolve by best judgment` |
| `the autonomous default` (a flag disambiguates the menu) | `a flag disambiguates every menu` |
| `Autonomous Default rule 6` (propagation) | `flag propagation to every delegate` |
| `Autonomous Default rule-4 exception list` | `the publish/irreversible exception list` (`core/ask-exception-list.md`) |
| `SKILL-SPEC §15` (advisory vs mandated) | `the mandated-blocker test: a citable external authority makes it a documented prerequisite whose omission causes rejection, legal exposure, or a broken production path` |
| `SKILL-SPEC § State Management` | `state-qualifying: a resumable multi-scope audit whose progress exists nowhere else` |
| `**IDU:**` | `**Upstream artifacts:**` |
| `DSC` / `FRC+DSC accounting` | `deterministic checklist` / `disposition accounting` |
| `OVERLAP-n` | the overlap in prose: `privacy audit belongs to ds-compliance` |
...
4. **Flag propagation.** When the orchestrator itself was invoked with `--ask`, it MUST pass `--ask` on every delegated invocation at the "invoke" step of rule 3; without it a delegate never prompts on its own. The orchestrator's own gates (Phase-level confirmations, Category A/B batches it presents itself) resolve per Autonomous Default identically to any other skill.
...
## Engineering Principles — one home

The engineering principles every skill internalizes (cross-cutting themes, SOLID + GRASP, Twelve-Factor, reliability, security baseline, process rules, testing discipline, configuration/secrets/data, the anti-overengineering guard, cross-scope dedup, needs-approval reason discipline) live in [`core/principles.md`](core/principles.md), each with a mechanical detection signal or an explicit `judgment` tag and a primary source. Skills link the section their phase needs; the full curated catalog behind it is `core/software-best-practices.md`. This spec does not restate them.
...
Present needs_approval items with risk context under `--ask`: Apply All / Review Each / Skip All. Default: resolved by best judgment, no prompt, recorded in the summary.
...
See the [AI Instruction Patterns appendix](#appendix-ai-instruction-patterns--research-rationale) for the research behind these patterns.
...
5. Shared text is linked, never copied: a skill reaches `core/` by relative path (`../core/<file>.md`) and never a sibling skill's directory; `core/` ships on every install, so the link resolves in a lone install (`check-consistency.sh` check `core-links`)
...
| E | Process carriers (orchestrators, not dimensions) | 11 skills |
...
| D6 | CI/CD & release engineering | ds-devops + ds-launch + ds-release | Pipeline integrity, code signing, staged rollout, user-facing release trust (changelog + rollback narrative) | [DORA](https://dora.dev/) (deployment frequency, lead time for changes) |
...
| E | Process carriers | ds-ship, ds-pipeline, ds-commit, ds-pr, ds-issue, ds-init, ds-build, ds-debug, ds-research, ds-brief, ds-freeze | Orchestration, spec pipeline, commits, PRs, release-scope triage — not quality dimensions | n/a — carrier, not a dimension |
...
| ds-fix | FIX | ds-compliance | CMP | ds-build | BLD |
...
## Appendix: AI Instruction Patterns — research rationale

§2 is the normative distillation of this research; this appendix is the dated evidence behind it (sources, measurements, and what was removed when a claim could not be traced). Nothing here is loaded at runtime.
