...
| Rule | Detail |
|------|--------|
| **Freshness** | `git_hash` ≠ current HEAD → stale; the consumer re-analyzes (a full-codebase analyzer overwrites the file; a partial analyzer overwrites only its own scopes). |
