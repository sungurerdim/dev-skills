...
- **Detect:** Search for padding/margin/gap values not in the project's spacing scale. Common scales: 4px-based (4/8/12/16/20/24/32/48/64) or 8px-based (8/16/24/32/48/64).
...
- **Fix:** Round to nearest scale value. If project has no defined scale, recommend 4px-based scale as starting point.
- **Impact:** Inconsistent spacing creates visual noise, slows design iteration, and makes responsive behavior unpredictable.
- **Source:** Material Design Spacing, Apple HIG Layout Margins

### TOK-03 [HIGH] Typography Scale
Defined type scale with consistent ratio (1.2-1.5), no ad-hoc font sizes outside the scale.
- **Detect:** Search for font-size values not matching the project's type scale. Check for random sizes like 13px, 15px, 17px that break scale consistency.
...
- **Impact:** Arbitrary shadows create inconsistent depth perception and make theming difficult.
...
- **Impact:** Semantic naming enables theming (dark mode swaps decision tier, not component code), improves discoverability for both humans and AI agents.
...
- **Fix:** Convert existing token files to DTCG format. Use Style Dictionary v4+ for transformation pipeline.
  ...
  ```
- **Impact:** Standard format enables tooling interoperability, automated token pipelines, and cross-platform synchronization.
...
- **Impact:** Unused tokens bloat the token set and confuse consumers. Untokenized values resist theming and centralized updates.
...
- **Detect:** Search for color values that use light-only tokens without dark equivalents. Check for white/light backgrounds without dark theme alternative.
...
- **Impact:** Overriding system preference frustrates users who have chosen their preferred mode for comfort or accessibility reasons.
...
- **Impact:** Dark themes frequently introduce contrast violations because designers test light mode first. Users with low vision are disproportionately affected.
...
- **Impact:** Bright images on dark backgrounds cause visual shock and discomfort, especially in low-light environments.
