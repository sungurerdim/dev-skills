...
| Check an installed copy for drift | `./install.sh --check` |
...
- **Completion Evidence band** appears verbatim at both the top and the bottom of every `SKILL.md`.
  Both copies are normative — do not "de-duplicate" them. The repeat is deliberate: model attention
  is position-biased, and the portable profile supports six hosts.
