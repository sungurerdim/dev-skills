...
**Gate:** Pass = topic + scope + currentDate resolved (audience → General when unanswered); branch-shaped topic → dimensions approved with exhaustive value sets incl. a catch-all. If too broad/ambiguous → ask 1 clarifying question; no answer after one re-prompt → assume Standard/research with the literal topic, warn, and proceed (currentDate → host date). **Under `--auto`:** no clarifying question — proceed directly with Standard/research on the literal topic, same warn recorded in the summary.
...
- Action list (ds-opt:todo) **whenever the brief tells the reader to do things** — rule-tagged `<li data-when>` items authored once (JS-off safe) and filtered by the branch layer's own evaluator; profile line + count + unanswered-questions note; copy-list and opt-in personal print (rules: report-template.md § Action list layer). Above 2 decision dimensions this **replaces** per-combination branch cards
