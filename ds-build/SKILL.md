...
- **Checkpoint** ([../core/checkpoint-protocol.md](../core/checkpoint-protocol.md)): `git status --porcelain` before the first write; a dirty path the plan would touch → that unit is `needs-human` (commit or stash first); disjoint dirt is listed and left alone. Failed units are reverted file-by-file (`git checkout -- {files}`), never with a tree-wide reset.
- **Mechanical Done Gate:** `{check-cmd}` resolved once at setup (ds-quality enforcement arm when installed, else the stack-native format → lint → type → test chain), baseline captured, re-run after every unit and once in aggregate before close; new red → fix ≤ 3 attempts with the same command, then revert the unit and record `reverted`; baseline red reported red-at-baseline; no tooling → Verification-Infrastructure Gap surfaced, never skipped.
...
- **Publishing is never done here:** commits are local; push, PR, tag, release, deploy are on the exception list ([../core/ask-exception-list.md](../core/ask-exception-list.md)) and reported `needs-human` with the exact command.
...
**Gate:** Source resolved, done set listed, every item carries a signal, baseline captured, tree checkpointed. If fails → no source found → stop with the three accepted forms; a done-set item that cannot get a signal → `needs-human: state how {item} is verified`, the rest proceeds.
...
**Gate:** Every done-set item owned by a unit; every unit has files + signal. If fails → unowned item → add a unit or record `needs-human: no unit can close {item} because {reason}`.
...
**Gate:** Each unit `done` (signal green + committed) or `blocked` with causes, never silently skipped. If fails → a commit is rejected by a hook → fix what it names and retry (≤ 3), never bypass the hook; a checkpoint conflict appears mid-run (new dirt on a planned path) → that unit `needs-human`, continue with the rest.
...
**Gate:** Record written; no done-set item without evidence; blocked items listed with causes. If fails → issue body write fails (permissions, `gh` down) → print the Closure block in chat and keep the issue open, `needs-human: paste the closure block`.
...
- Commits are local; push / PR / tag / release stay `needs-human`
...
| Unit needs a value only a human has (credential, business rule) | `needs-human` with the exact question; dependent units wait, independent units continue |
| Plan wants a publishing step (push, tag, release) | Recorded `needs-human` with the command; the local work is committed |
