...
ran=$(grep -iE 'ds-(launch|benchmark|productize)' "$rep" 2>/dev/null | grep -viE 'skipped|not run|excluded|not applicable|n/a|no signal|no billing|no store' | wc -l | tr -d ' ')
rc=1; [ "${ran:-1}" = "0" ] && rc=0
...
rc=1; [ -z "$(git status --porcelain | grep -vE '^\?\? ds/$|^\?\? (CLAUDE|AGENTS|GEMINI)\.md$')" ] && rc=0
r "no-stray-source-edits(ds/ + instruction file allowed)" "$rc"
